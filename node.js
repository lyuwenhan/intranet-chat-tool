...
function runcpp(command, cpp, input, output, errfile, callback, token){
	...
	cpp_runlist = cpp_runlist.then(async () => {
		...
		let result;
		try{
			result = await start_runcpp(command, ["run", "--rm", "--network", "none", "--read-only", "--pids-limit=64", "--cpus=0.5", "--memory=130m", "--cap-drop=ALL", "--security-opt", "no-new-privileges", "--tmpfs", "/tmp", "--tmpfs", "/app/data", "-v", `${path.resolve(userFileDir)}:${userFileDirInDocker}:rw`, "judge-runner", `${userFileDirInDocker}/user.cpp`, `${userFileDirInDocker}/user.in`, `${userFileDirInDocker}/user.out`, `${userFileDirInDocker}/user.err`, `/app/user.run`, String(timeout), "128", "1048576", "-O2"]);
		}finally{
			try{
